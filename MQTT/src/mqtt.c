//created by Ankush Sharma on 13th September 2017
...
#include <fcntl.h> //fcntl() can be used to retrieve or modify open file flags given a file descriptor
#include <assert.h>//can be used to verify assumptions made by the program and print a diagnostic message if this assumption is false
#include <sys/time.h>
...
mqtt_broker_handle_t * mqtt_connect(const char* client, const char * server_ip, uint32_t port)
{
    ...
    if(broker != 0) {
        ...
        if ((broker->socket = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
